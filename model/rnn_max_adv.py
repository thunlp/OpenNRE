...
def rnn_max_adv(is_training):
    if is_training:
        with tf.variable_scope('rnn_max_adv', reuse=False): 
        ...
        # Train
        with tf.variable_scope('rnn_max_adv', reuse=True): 
            x = framework.encoder.rnn(new_embedding, FLAGS.hidden_size, framework.length)
            logit, repre = framework.selector.maximum(x, framework.scope)
            loss = framework.classifier.softmax_cross_entropy(logit)
