...
def softmax_cross_entropy(x, label, rel_tot, var_scope=None):
    with tf.variable_scope(var_scope or "loss", reuse=tf.AUTO_REUSE):
        weights_table = tf.get_variable("weights_table", dtype=np.float32, shape=(rel_tot), initializer=tf.ones_initializer)
