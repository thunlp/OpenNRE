    ...
    with tf.variable_scope(var_scope or "attention", reuse=tf.AUTO_REUSE):
        if is_training: # training
            ...
            for i in range(scope.shape[0]):
                ...
                attention_logit = __attention_train_logit__(bag_hidden_mat, tf.ones((bag_hidden_mat.shape[0]), dtype=tf.int32) * query[i], rel_tot)
                attention_score = tf.nn.softmax(attention_logit, axis=-1)
            ...
            for i in range(scope.shape[0]):
                bag_hidden_mat = x[scope[i][0]:scope[i][1]]
                attention_score = tf.nn.softmax(tf.transpose(attention_logit[scope[i][0]:scope[i][1], :]), axis=-1) # softmax of (rel_tot, n')
                ...
                bag_logit.append(tf.diag_part(tf.nn.softmax(bag_logit_for_each_rel, axis=-1))) # could be improved by sigmoid?
            bag_repre = tf.stack(bag_repre)
...
def bag_average(x, scope, rel_tot, var_scope=None, dropout_before=False):
    with tf.variable_scope(var_scope or "average", reuse=tf.AUTO_REUSE):
        ...
        for i in range(scope.shape[0]):
            bag_hidden_mat = x[scope[i][0]:scope[i][1]]
            bag_repre.append(tf.reduce_mean(bag_hidden_mat, axis=0)) # (n', hidden_size) -> (hidden_size)
        bag_repre = tf.stack(bag_repre)
        if not dropout_before:
            bag_repre = __dropout__(bag_repre)
    return __logit__(bag_repre, rel_tot), bag_repre
...
def bag_maximum(x, scope, query, rel_tot, is_training, var_scope=None, dropout_before=False): # could be improved?
    with tf.variable_scope(var_scope or "maximum", reuse=tf.AUTO_REUSE):
        if self.is_training: # training
            ...
            for i in range(scope.shape[0]):
                bag_hidden_mat = x[scope[i][0]:scope[i][1]]
                instance_logit = tf.nn.softmax(__logit__(bag_hidden_mat, rel_tot), axis=-1) # (n', hidden_size) -> (n', rel_tot)
            ...
            for i in range(scope.shape[0]):
                bag_hidden_mat = x[scope[i][0]:scope[i][1]]
                instance_logit = tf.nn.softmax(__logit__(bag_hidden_mat, rel_tot), axis=-1) # (n', hidden_size) -> (n', rel_tot)
                bag_logit.append(tf.reduce_max(instance_logit, axis=0))
