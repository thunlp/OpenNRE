...
class rl_re_framework(framework.re_framework):
    ...
    def agent_one_step(self, sess, agent_model, batch_data, run_array, weight=1):
        feed_dict = {
            agent_model.word: batch_data['word'],
            agent_model.pos1: batch_data['pos1'],
            agent_model.pos2: batch_data['pos2'],
            agent_model.label: batch_data['agent_label'],
            agent_model.length: batch_data['length'],
            agent_model.weight: weight
        }
    ...
    def train(self,
        ...
        train_main_model(self, max_epoch=5) # Pre-train main model
        train_agent_model(self, max_epoch=1) # Pre-train policy agent 
        ...
        for epoch in range(max_epoch):
            ...
            for i, batch_data in enumerate(self.train_data_loader):
                ...
                iter_logit = self.agent_one_step(self.sess, self.agent_model, batch_data, [self.agent_model.train_logit()])
                ...
                iter_loss = self.one_step(self.sess, self.model, batch_data, [self.model.loss()])
                reward += iter_loss
                ...
                if batch_count == 100:
                    ...
                    for batch_data in batch_stack:
                        ...
                        self.agent_one_step(self.sess, self.agent_model, batch_data, [self.agent_train_op], weight=reward * alpha)
                    batch_count = 0
                    reward = 0
                    tot_delete = 0
                    batch_stack = []
