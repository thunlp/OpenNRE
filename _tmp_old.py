...
class pcnn_att_npy(nrekit.framework.re_model):
    def __init__(self, train_data_loader, batch_size, max_length=120):
        ...
        x_train = nrekit.network.encoder.pcnn_with_mask(x, self.mask, keep_prob=0.5)
        x_test = nrekit.network.encoder.pcnn_with_mask(x, self.mask, keep_prob=1.0)
        # x_train = nrekit.network.encoder.pcnn(x, self.pos1, self.pos2, self.length, keep_prob=0.5)
        # x_test = nrekit.network.encoder.pcnn(x, self.pos1, self.pos2, self.length, keep_prob=1.0)
...
f.train(pcnn_att_npy, ckpt_dir='tmp_ckpt', model_name='pcnn_att_npy', max_epoch=40, gpu_nums=1)
